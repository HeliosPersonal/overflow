    ...
    ILogger<QuestionsController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<Question>> CreateQuestion(CreateQuestionDto dto)
    {
        ...
        await InvalidateQuestionListCacheAsync();
    ...
    public async Task<ActionResult<PaginationResult<Question>>> GetQuestions([FromQuery] QuestionsQuery q)
    {
        ...
        var result = await cache.GetOrSetAsync(cacheKey, async _ =>
        {
    ...
    public async Task<ActionResult> UpdateQuestion(string id, CreateQuestionDto dto)
    {
        ...
        {
            logger.LogWarning(
                "Update forbidden: User {UserId} attempted to update question {QuestionId} owned by {OwnerId}",
        ...
        await InvalidateQuestionListCacheAsync();
        await cache.ExpireAsync(CacheKeys.QuestionDetail(id));
    ...
    public async Task<ActionResult> DeleteQuestion(string id)
    {
        ...
        {
            logger.LogWarning(
                "Delete forbidden: User {UserId} attempted to delete question {QuestionId} owned by {OwnerId}",
                userId, id, question.AskerId);
            return Forbid();
        }
        ...
        await InvalidateQuestionListCacheAsync();
        await cache.RemoveAsync(CacheKeys.QuestionDetail(id));
    ...
    public async Task<ActionResult> PostAnswer(string questionId, CreateAnswerDto dto)
    {
        ...
        await InvalidateQuestionListCacheAsync();
        await cache.ExpireAsync(CacheKeys.QuestionDetail(questionId));
    ...
    public async Task<ActionResult> DeleteAnswer(string questionId, string answerId)
    {
        ...
        {
            logger.LogWarning(
                "Answer delete forbidden: Answer {AnswerId} is accepted or doesn't match question {QuestionId}",
                answerId, questionId);
    ...
    public async Task<ActionResult> AcceptAnswer(string questionId, string answerId)
    {
        ...
        {
            logger.LogWarning(
                "Answer accept failed: Mismatch or already has accepted answer. Question={QuestionId}, Answer={AnswerId}",
                questionId, answerId);
    ...
    private async Task InvalidateQuestionListCacheAsync()
    {
        await cache.RemoveByTagAsync(CacheTags.QuestionList);
        logger.LogDebug("Question list cache invalidated");
    }
}