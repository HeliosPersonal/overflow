...
public static class WebApplicationBuilderExtensions
{
    ...
    public static IHostApplicationBuilder AddEnvVariablesAndConfigureSecrets(this IHostApplicationBuilder builder)
    {
        ...
        try
        {
            ...
            logger.LogInformation("Authenticated with Infisical successfully");
            ...
            foreach (var path in AppSecretPaths)
            {
                try
                {
                    var secrets = ListSecretsFromPath(infisicalClient, environmentSlug, projectId, path);
                    logger.LogInformation("Loaded {Count} secrets from Infisical path: {Path}", secrets.Length, path);
                    allSecrets.AddRange(secrets);
                }
                catch (Exception pathEx)
                {
                    logger.LogError(pathEx, "Failed to load secrets from Infisical path: {Path}", path);
                    throw;
                }
            }
            ...
            {
                logger.LogWarning(
                    "No secrets found in Infisical for environment '{Environment}' — checked paths: {Paths}",
                    environmentSlug, string.Join(", ", AppSecretPaths));

                if (builder.Environment.IsProduction())
                {
                    throw new InvalidOperationException(
                        $"No secrets returned from Infisical for environment '{environmentSlug}'. " +
                        $"Check that the machine identity has read access to the production environment " +
                        $"and that secrets exist under: {string.Join(", ", AppSecretPaths)}");
                }
            }
        }
        catch (Exception ex)
    ...
    private static Secret[] ListSecretsFromPath(InfisicalClient client, string environmentSlug,
        string projectId, string secretPath)
    {
        ...
        return client.Secrets().ListAsync(options).GetAwaiter().GetResult();
    }
}