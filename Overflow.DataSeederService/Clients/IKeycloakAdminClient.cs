...
{
    [Post("/users")]
    Task<HttpResponseMessage> CreateUserAsync([Body] KeycloakCreateUserRequest body, CancellationToken ct = default);

    [Get("/users")]
    Task<List<KeycloakUserDto>> GetUserByUsernameExactAsync(
        [Query] string username, [Query] bool exact = true, CancellationToken ct = default);
}
